...
public class AssociationTransition extends Transition{
	
	@Override
	public RelationView createView() {
		RelationView relationView = null;
		try {
			relationView = (RelationView)Thread.currentThread().getContextClassLoader().loadClass("org.uengine.kernel.designer.ui.AssociationTransitionView").newInstance();
